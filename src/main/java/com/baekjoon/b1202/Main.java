...
public class Main {
    ...
    public static void main(String... arg) throws Exception{
        ...
        int localc;
        for (int i = 0; i < K; i++) {
            localc = Integer.parseInt(br.readLine());
            if( ts.contains(localc)) {
                map.put(localc, map.get(localc) + 1 );
            } else {
                ts.add(localc);
                map.put(localc, 1);
            }
        ...
        for (int i = N-1; i >= 0; i--) {
            localm = (int)(vm[i] % VMDUMMY);
            localv = (int)(vm[i] / VMDUMMY);
            // 보석을 넣을 수 있는 가장 작은 가방 선택
            bagc = ts.ceiling(localm);
            // 넣을 수 있는 가방이 있다면
            if( bagc != null ) {
                if( map.get(bagc) > 1 ) {
                    map.put(bagc, map.get(bagc)-1);
                } else {
                    ts.remove(bagc);
                }
                sum += localv;
